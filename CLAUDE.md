...
## Tech stack (confirmed — see `ROADMAP.md` §4 for full detail)

- Language/runtime: **Python 3.11+**, managed with **uv**. Simulation state
  uses `dataclasses`, plain dicts/lists/sets, and explicit type hints —
  **no pandas/numpy in simulation state management** (SRS §8, ROADMAP §4.2,
  §6.7). Tests via **pytest**.
- API / application layer: **FastAPI**. Sits between the Streamlit dashboard
  and the simulation engine; never lets the dashboard talk to the DB or Kafka
  directly. Introduced at Phase 8 (Step 20) — don't build it earlier than
  that stage.
- Dashboard: **Streamlit**. UI only — must never become the simulation
  engine (ROADMAP §4.6).
- Local infra: **Docker Compose** for Postgres + Kafka (Redpanda is a lighter
  Kafka-compatible alternative if resources are tight). Full Dockerization
  (Dockerfiles, health checks, etc.) is Stage/Step 30 — last milestone, not
  needed until then.
- Streaming: **Kafka**, or **Redpanda** as the Kafka-compatible local
  alternative (ROADMAP §4.3).
- Operational DB: **PostgreSQL** — current state (ROADMAP §4.4).
- Analytical warehouse: **Snowflake**; a local stand-in (e.g. DuckDB) may be
  used during development until Stage 9 — decide the stand-in when you get
  there, not before.
- AI agents: **Google Gemini (Flash, free tier) via the `google-genai` SDK**
  (not Claude Code itself, and not Anthropic — switched from the original
  spec to use a free-tier provider for the certificate submission; see
  `SCOPE.md`) — agent code calls the API directly with structured/
  function-calling output for proposing actions. Config via `GEMINI_API_KEY`
  / `GEMINI_MODEL` env vars. Free-tier rate limits are low: `agents/base.py`
  must retry/back off on 429s, and automated tests use a mocked response
  rather than live calls (ROADMAP §4.7).
- Godot: version _TBD_ — only relevant if/when a client beyond the Streamlit
  dashboard is added; integrates via the API layer only, never talks to the
